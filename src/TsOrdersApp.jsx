...
const TsOrdersApp = () => {
  ...
  const {
    ...
    setOrders,
  } = useOrderResources(ORDER_FILTERS);
  ...
  const {
    ...
    handleSwitchChange,
  } = useOrderSwitches({ orders, setFilters: setItemsFilter, removeShipment });
  const formattedAddress = useAddressFormatter(addressToFormat);
  const updateShipmentSwitch = useCallback(
    (id, value) => setSwitchStates((states) => ({ ...states, [id]: value })),
    [setSwitchStates]
  );
  ...
  return (
    <div className="content">
      <Header
        handlerModalSearch={() => setIsOpenModal((open) => !open)}
